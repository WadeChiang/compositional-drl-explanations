...
def gen_feat_mask(inputs) -> np.ndarray:
    ops = settings.blackjack_operators
...
def search_feats(acts, feat_masks, feat_names, weights):
    ...
    with pool_cls(settings.PARALLEL) as pool, tqdm(
        ...
        for res in pool.imap_unordered(ioufunc, mp_args):
            ...
            w_stick = weights[unit, 0]
            w_hit = weights[unit, 1]
            if best_iou > 0:
...
def feat2name() -> list[str]:
    names = list(settings.blackjack_operators.keys())
    names.append("Null")
    return names
...
def main(
    ckpt_path="",
    data_path="/root/gym/rl_compexp/save/Blackjack2-DQN64",
):
    ...
    print(mean_iou)
    res.to_csv(f"{data_path}/result_5_parsed_.csv", index=False)
