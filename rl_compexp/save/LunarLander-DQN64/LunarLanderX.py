...
class LunarLanderX(gym.Env, EzPickle):
    metadata = {
    ...
    def reset(
            ...
            *,
            ...
            state: Optional[np.ndarray] = None,  # 新添加的state参数
        ):
            ...
            if state is not None:
                initial_x = float(state[0]) * (VIEWPORT_W/SCALE/2) + (VIEWPORT_W/SCALE/2)
                initial_y = float(state[1])  * (VIEWPORT_H/SCALE/2) + (self.helipad_y + LEG_DOWN / SCALE)
                initial_angle = float(state[4])  # 直接使用状态中的角度
            else:
            ...
            # 设置速度
            if state is not None:
                self.lander.linearVelocity.x = float(state[2])  # vx
                self.lander.linearVelocity.y = float(state[3])  # vy
                self.lander.angularVelocity = float(state[5])   # angular velocity
            else:
                # 原来的随机初始化
            ...
            for i in [-1, +1]:
                # 计算考虑旋转后的腿的位置
                leg_local_x = -i * LEG_AWAY / SCALE
                leg_local_y = 0
                
                # 使用旋转矩阵计算旋转后的位置
                cos_theta = math.cos(initial_angle)
                sin_theta = math.sin(initial_angle)
                leg_rotated_x = leg_local_x * cos_theta - leg_local_y * sin_theta
                leg_rotated_y = leg_local_x * sin_theta + leg_local_y * cos_theta
                
                # 最终的腿的位置是着陆器位置加上旋转后的相对位置
                leg_x = initial_x + leg_rotated_x
                leg_y = initial_y + leg_rotated_y

                leg = self.world.CreateDynamicBody(
                    position=(leg_x, leg_y),
                    angle=(initial_angle + i * 0.05),
                    fixtures=fixtureDef(
                ...
                leg.ground_contact = False if state is None else bool(state[6 if i==-1 else 7])
...
if __name__ == "__main__":
    ...
    gym.register(
    id="LunarLanderX-v0",
    entry_point=LunarLanderX,
    )
    env = gym.make("LunarLanderX-v0", render_mode="rgb_array")
    print(env.reset())
