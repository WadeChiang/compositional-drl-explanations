...
# ckpt = "/root/gym/rl_compexp/save/LunarLander-A2C1024"
ckpt = None
save_path = "../save/LunarLander-A2C1024"


def save_best_fn(policy: A2CPolicy):
    # 保存actor和critic的状态字典
    state = {"model": policy.state_dict(), "optim": policy.optim.state_dict()}
    torch.save(state, os.path.join(save_path, "a2c_best.pth"))


if ckpt:
    pth = torch.load(os.path.join(ckpt, "a2c_best.pth"))
    print(pth)
...
torch.save(state, os.path.join(save_path, "a2c.pth"))
